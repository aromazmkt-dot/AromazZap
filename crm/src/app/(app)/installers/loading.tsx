      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-zinc-100 animate-pulse rounded-xl p-4 space-y-2">
            <Sk w="w-20" h="h-3" /><Sk w="w-12" h="h-7" /><Sk w="w-24" h="h-3" />
          </div>
        ))}
      </div>
      <div className="bg-white border border-zinc-100 rounded-xl overflow-hidden">
        <div className="flex gap-4 px-4 py-3 bg-zinc-50 border-b border-zinc-100">
          {['w-28', 'w-24', 'w-20', 'w-24', 'w-20', 'w-20', 'w-16'].map((w, i) => (
        ...
        {[...Array(7)].map((_, i) => (
          <div key={i} className="flex gap-4 px-4 py-4 border-b border-zinc-50 items-center">
            <div className="w-8 h-8 rounded-full bg-zinc-100 animate-pulse flex-shrink-0" />
            <Sk w="w-28" h="h-3.5" /><Sk w="w-24" h="h-3" /><Sk w="w-20" h="h-6" />
            <Sk w="w-24" h="h-3.5" /><Sk w="w-20" h="h-3" /><Sk w="w-16" h="h-5" />
          </div>
        ))}
      </div>
    </div>
